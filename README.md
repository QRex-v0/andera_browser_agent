...
- Two individually correct fixes combined into a regression, twice. Loop detection that ignored the `navigate` verb was emptied when click-to-navigate rewrote clicks as navigate. URL validation that rejected raw whitespace also rejected `+` in query strings — the standard encoding for a space — so a valid GitHub search URL was refused and the agent paginated into a secondary rate limit. Each guard has to be tested against the legal cases it must still allow.
